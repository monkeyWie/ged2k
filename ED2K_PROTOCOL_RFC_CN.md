...
- **协议标识符（1字节）**：协议类型标识
  ...
  - `0xC5`：eMule 扩展协议
  - `0xD4`：压缩协议
- **大小（4字节，小端序）**：数据包总大小，包括操作码（不包括协议+大小字段）
- **操作码（1字节）**：数据包类型标识符
- **载荷数据（大小-1字节）**：特定于数据包类型的数据
...
**字段描述：**
- **IPv4 地址（4字节，小端序）**：网络地址
- **端口（2字节，小端序）**：TCP/UDP 端口号
...
**字段描述：**
- **客户端哈希（16字节）**：持久的客户端标识符（用户哈希）
- **客户端IP（4字节，小端序）**：客户端的公网IP地址
- **客户端端口（2字节，小端序）**：客户端的监听端口
- **标签数量（4字节，小端序）**：能力标签的数量
- **标签数据（变长）**：客户端能力和版本标签
